...
public class readingXMLCurrency {
	...
	public static void display(ArrayList<Currency> currencies){
		...
		//else use printf to print out something like   $1.00 in USD is worth 20.47 MXN
	}
	public static ArrayList<Currency> getCurrencyData(){
		...
		try{
			...
			//assign document = DocumentBuilderFactory.newInstance().newDocumentBuilder().parse(new URL(url).openStream());
			//get a nodelist from document where elements tag name = "Cube" and getAttributes().getLength() == 2
		...
		//add an additional Currency to currencies array 
		//                  where name = "EUR" and value = 1d
		//                  because there is none in .xml file see instructions
		return currencies;
	}
